...
for (let i = array.length - 1; i >= 0; i--) {
    if (i % 2 === 0) {
        newCard.unshift(array[i] * 2);
    } else {
        newCard.unshift(array[i]);
    }
}

for (let i = 0; i < newCard.length; i++) {
    if (newCard[i] >= 10) {
        let mod = newCard[i] % 10;
        let div = Math.floor(newCard[i] / 10);
        sum = sum + mod + div
...
let validity = sum % 10 === 0 ? 'valid' : 'invalid';
return validity;
}
...
const invCards = findInvalidCards(batch);
console.log("Calling findInvalidCards: ")
